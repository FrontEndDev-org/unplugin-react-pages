...
import { createLogger, normalizePath, type Logger, type ResolvedConfig, type ViteDevServer } from 'vite';
...
export class ReactApp {
    ...
    private _isTreeFile(path: string) {
    ...
    async setup() {
        ...
        if (pageFileNames.length > 0) {
            const pageFiles = await glob(nodePath.join('**', pageFileNames.length > 1 ? '{' + pageFileNames.join(',') + '}' : pageFileNames[0]), {
                cwd: this.absApp,
                nodir: true,
                dot: false,
